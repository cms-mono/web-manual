...
const HZ_HOME_KINDS = [
  {
    ...
    types: [
      ...
      ["MMS", "이미지와 문구가 포함된 메시지를 바로 작성할 때", "direct", "step-rcs-flowmms-1"],
      ["템플릿", "미리 등록한 템플릿을 불러와 발송할 때", "pre", "step-rcs-flowtmpl-1"],
...
const HZ_SRC = [
  { part: "brand",    title: "브랜드 · 발신번호 선택",
    ...
    detail: [
      { t: "\u2460 브랜드 선택 — 오른쪽 [조회]를 누릅니다", hi: ["브랜드 선택"],
        d: "브랜드는 직접 입력할 수 없습니다. 칸 오른쪽 끝의 <b>[조회]</b> 버튼을 눌러 목록에서 고릅니다." },
      { t: "\u2461 목록이 비어 있다면 — 아직 등록·승인 전입니다", hi: ["브랜드 선택"], pop: "brandEmpty",
        d: "팝업은 떴는데 표가 비어 있다면 RBC(RCS 비즈니스센터) 쪽을 확인합니다.<br>· RBC 사이트에서 <b>브랜드를 등록</b>했는지, 그리고 <b>승인이 완료</b>되었는지<br>· <b>RBC 사이트 &gt; 브랜드 운영 관리</b>에서 <b>KT에 대행사 운영권한을 부여</b>했는지<br>둘 중 하나라도 빠져 있으면 이 목록에 나오지 않습니다.<br>대행사 운영권한은 <b>부여 직후가 아니라 서버 동기화(약 3~4시간) 뒤</b>에 반영됩니다.<br>→ <a class=\"xref\" data-to=\"rbc\" data-sec=\"step-rcs-brand-1\">RBC 브랜드 개설</a> · <a class=\"xref\" data-to=\"rbc\" data-sec=\"step-rcs-agency-1\">대행사 운영권한 부여</a>" },
      { t: "\u2462 등록·승인이 끝났다면 이렇게 나옵니다", hi: ["브랜드 선택"], pop: "brand",
        d: "표는 <b>번호 · 브랜드 명 · 브랜드 ID</b> 3열이고 위에 검색창이 있습니다. <b>내 계정에 등록·승인된 브랜드만</b> 나옵니다. 한 줄을 고르고 <b>[확인]</b>.<br><b class=\"hz-ex\">이 화면에서는</b> <code>모노커뮤니케이션즈</code> 를 골랐습니다." },
      { t: "\u2463 발신 번호 — 대표번호가 자동으로 들어갑니다", hi: ["발신 번호"],
        d: "브랜드를 고르면 <b>대표 발신번호가 자동으로 선택</b>됩니다. 그대로 보내도 되고, <b>다른 번호로 바꾸고 싶을 때만</b> 칸 오른쪽의 <b>[조회]</b>를 눌러 고릅니다." },
      { t: "\u2464 목록이 비어 있다면 — 대화방이 아직입니다", hi: ["발신 번호"], pop: "numberEmpty",
        d: "브랜드는 있는데 발신번호만 안 나오는 경우입니다. 그 브랜드에 <b>대화방을 등록</b>했는지, 그리고 <b>승인이 완료</b>되었는지 확인합니다.<br>→ <a class=\"xref\" data-to=\"rbc\" data-sec=\"step-rcs-room-1\">대화방(발신번호) 등록</a>" },
      ...
      { t: "\u2467 대화방 유형 두 가지", hi: ["발신 번호"], pop: "number",
        d: "아래 목록에서 <b>1·2번만 <code>bot-</code> 코드</b>이고 <b>나머지는 전화번호 형식</b>입니다(3·5번은 자사 번호, 4·6번은 가림 처리). <b>발신번호</b>로 등록하면 문자를 보내는 번호 그 자체가 대화방이 되어 <b>ID가 번호 그대로</b>입니다.<br><b>양방향 ID</b>로 등록하면 하나의 발신번호 아래 <b>여러 ID를 만들어</b> 대화방명과 대화방 메뉴를 다르게 노출할 수 있고, 번호 대신 발급된 코드가 ID가 되어 <code>bot-</code>으로 시작합니다.<br>같은 번호를 쓰면서 부서·브랜드별로 이름을 달리 보여주고 싶을 때 양방향 ID를 씁니다.",
        table: { head: ["대화방 유형", "목록에 보이는 ID", "쓰는 상황"], rows: [
          ["발신번호", "15777223 · 023337223", "번호 하나에 대화방 하나. 가장 일반적인 형태"],
          ["양방향 ID", "bot-**********", "번호 하나에 여러 대화방. 대화방명·메뉴를 따로 노출"],
        ] } },
    ] },
  ...
  { part: "msgtype",  title: "메시지 종류 선택",
    ...
    detail: [
      { t: "① 메시지 종류 — [조회]를 누릅니다", hi: ["메시지 종류"],
        d: "메시지 종류를 먼저 골라야 아래 제목·내용 입력란이 열립니다. 칸 오른쪽의 <b>[조회]</b>를 누릅니다.<br>고르기 전에는 제목·내용 칸이 <b>비활성 상태</b>로 <b>필수(*)</b> 표시만 붙어 있습니다. 베이스를 고르면 그 유형에 맞는 항목으로 화면이 다시 그려지므로, 이 표시는 그대로 믿지 않아도 됩니다(예: LMS를 고르면 제목에 <b>사용/미사용</b>이 생기고 필수 표시는 없어집니다)." },
      ...
      { t: "템플릿은 종류가 많습니다",
        d: "헤르메스 <b>템플릿 관리 &gt; 템플릿 생성</b>에는 <b>이미지 8종 · LMS 4종 · 텍스트 6종</b>이 있습니다. 만드는 절차는 <b>템플릿 관리</b> 쪽에서 따로 다룹니다." },
    ] },
  ...
  { part: "common",   title: "공통 정보 · 제목 · 내용",
    ...
    detail: [
      ...
      { t: "무료 수신거부번호 — 필수", hi: ["무료 수신거부번호"],
        d: "툴팁은 <b>footer</b> — 본문 끝에 붙습니다. 입력란에 직접 적으며 안내 예시는 <code>08012345678</code>. <b>화면에서 번호를 만들어 주지는 않으므로</b> 쓸 080 번호가 미리 개통돼 있어야 합니다.<br><b class=\"hz-ex\">이 화면에서는</b> <code>08012345678</code> 을 넣었습니다. 미리보기에는 <code>수신거부 …</code> 로, 실제 수신 화면에는 <code>무료 수신거부 …</code> 로 붙습니다." },
      ...
      { id: "actbtn", t: "액션 버튼 설정", hi: ["액션 버튼 설정"],
  ...
  { part: "fallback", title: "발송 실패 시 문자 전송 (Fallback)",
    ...
    detail: [
      ...
      { t: "설정 여부 확인 경로", hi: ["사용 여부"],
        d: "툴팁 그대로 — <i>마이페이지 &gt; 청약관리 &gt; 발송포탈RCS_ID(ex&gt;P_RCSID_01) 상세 화면에서 확인 가능</i>. 이 화면에서는 <b>확인만</b> 됩니다. 설정을 바꾸려면 <b>KT RCS로 요청</b>해야 합니다." },
      { t: "켜면 무엇을 더 채우나",
        d: "<b>사용</b>으로 바꾸면 대체 발송으로 내보낼 문자(SMS/LMS/MMS) 내용을 따로 작성합니다." },
    ] },

  { part: "name",     title: "메시지 저장 · 표기 의무 확인",
    desc: "이름을 붙여 저장하면 다음에 그대로 불러 쓸 수 있습니다. 바로 아래에 <b>광고성 표기 의무 확인</b> 체크가 있습니다.",
    tip: "<b>메시지를 저장해야 발송할 수 있습니다.</b> 저장은 건너뛸 수 없는 단계입니다.",
    detail: [
      ...
      { t: "저장하지 않으면 — 발송 자체가 막힙니다", hi: ["메시지 이름"],
        d: "<b class=\"hz-warn\">메시지 저장은 선택이 아니라 필수입니다.</b> 저장하지 않고 아래 <b>[발송]</b>을 누르면 <i>메시지를 저장해야 발송이 가능합니다</i> 안내가 뜨고 더 진행되지 않습니다. (2026-09-08 실제 발송으로 확인)" },
      { t: "\u26a0 저장하면 아래 입력이 초기화됩니다", hi: ["메시지 이름"],
        d: "<b>[메시지 저장]</b>을 누르면 <b>수신자 목록 · 발송그룹ID · 표기 의무 체크</b>가 지워집니다.<br>그래서 <b class=\"hz-warn\">저장을 먼저 하고, 그 다음에 발송 정보와 수신자를 채우는 순서</b>로 진행해야 합니다. 이 따라하기의 단계 순서(작성 \u2192 저장 \u2192 발송 정보 \u2192 수신자 \u2192 발송)가 그 순서입니다." },
  ...
  { part: "sendinfo", title: "발송 정보 (형태 · 발송량 · 그룹ID)",
    ...
    detail: [
      ...
      { t: "② [발송량 현황 조회] 팝업 — 조회 범위", hi: ["발송량 현황 조회"], pop: "sendState",
      ...
      { t: "③ [발송량 현황 조회] — 시간대 보기", hi: ["발송량 현황 조회"], pop: "sendState",
        d: "<b>오전 08:00 ~ 오후 20:50</b>까지 <b>10분 간격</b>으로 펼쳐집니다. 계정별 설정이 아니라 <b>포탈 공통</b>으로, 21시가 넘어 배치성 광고가 나가지 않도록 막아 둔 것입니다. <b>[야간시간대 보기]</b>를 켜면 그 밖의 시간대도 보이고, <b>refresh</b>로 최신 수치를 다시 받습니다." },
      { t: "④ [발송량 현황 조회] — 칸 색깔 3가지", hi: ["발송량 현황 조회"], pop: "sendState",
        d: "<b>선택 가능</b> · <b>부분 발송 가능</b> · <b>선택 불가</b>. 부분 발송 가능인 칸에는 <code>12:30 / 38만건</code>처럼 남은 가용량이 함께 적히고 <b>[부분 발송 가용량 펼쳐보기]</b>로 한 번에 볼 수 있습니다." },
      { t: "발송량 — 단건과 대용량", hi: ["발송량"],
        d: "<b>단건/20건 이하 발송</b> 과 <b>대용량 발송(1회 10만건까지 발송 가능)</b> 중에서 고릅니다. 10만 건을 넘기려면 나눠서 발송해야 합니다." },
      { t: "발송그룹ID는 무엇에 쓰나", hi: ["발송그룹ID"],
        d: "발송 캠페인을 묶는 이름표입니다. <b>통신사별 100건 이상 전송에 성공</b>하면 이 ID로 RCS 비즈센터에서 <b>읽음확인·버튼클릭</b> 같은 고객반응 통계를 볼 수 있습니다. <b>발송할 때마다 다른 값</b>이어야 하고, 비워 두면 자동으로 채워집니다." },
      { t: "[중복검사] · [복사하기]", hi: ["발송그룹ID"],
        d: "검사 전에는 옆에 <b>‘발송그룹ID 중복검사필요’</b>가 빨갛게 남아 있습니다. <b>[중복검사]</b>를 눌러 통과시켜야 하고, <b>[복사하기]</b>로 방금 쓴 ID를 클립보드에 담아 둘 수 있습니다(나중에 통계에서 찾을 때 씁니다).<br>길이는 <b>20byte까지</b>입니다. 넘기면 <i>발송그룹ID는 20byte 이하만 입력가능합니다</i> 경고창이 뜹니다.<br><b class=\"hz-ex\">이 화면에서는</b> <code>GUIDE-20260827-01</code> 로 검사를 통과해 <b>사용가능</b>으로 바뀌었습니다." },
    ] },
  ...
  { part: "recv",     title: "수신자 추가",
    ...
    detail: [
      { t: "① 수신자를 넣는 방법은 세 가지", hi: ["목록"],
        d: "목록 오른쪽 위에 <b>[수신자 추가]</b> · <b>[커스텀 수신 정보 업로드]</b> · <b>[주소록 불러오기]</b> 세 버튼이 있습니다. 하나씩 눌러 보겠습니다." },
...
const HZ_GROUPS = [
  ...
  { id: "write", title: "메시지 작성",            kind: "유형별", from: ["common", "fallback", "name"],
    ...
    tip: "<b>'(광고)' 표시 여부</b>와 <b>무료 수신거부번호</b>는 필수 항목(*)입니다. 그리고 <b>메시지를 저장해야 발송할 수 있습니다.</b>",
...
const HZ_TYPES = {
  ...
  tmpl: {
    id: "tmpl", label: "템플릿", spec: "기존 RCS", base: "UBR.…",
    shot: "assets/hermes/result-tmpl.png",
    shotAlt: "기존 RCS · 템플릿으로 발송한 인증 메시지가 수신자 단말에 도착한 화면",
    shotCap: "기존 RCS · 템플릿(모노_인증메시지) 수신 화면 · 인증번호는 테스트 값입니다 (2026-09-08 실제 발송)",
    tip: { write: "템플릿은 문구가 정해져 있어 <b>제목·내용을 쓰지 않습니다.</b> 본문 한도 <b>90자</b>는 <b>고정 문구까지 합쳐</b> 세므로, 변수 카운터만 보면 안 됩니다(초과 시 <b>55709</b>)." },
    set: {
      msgtype: [["규격", "기존 RCS"], ["종류", "템플릿"], ["템플릿", "모노_인증메시지"]],
      write: [["'(광고)' 표시", "미사용"], ["무료 수신거부번호", "비움(선택)"], ["만료 옵션", "40초"],
              ["변수 cell1", "151512"], ["변수 총합", "6 / 90"], ["Fallback", "미사용"]],
      sendinfo: [["발송 형태", "즉시 발송"], ["발송량", "단건 · 20건 이하"], ["발송그룹ID", "GUIDE-20260908-04"]],
    },
    detail: {
      base: { d: "고른 규격에 따라 아래 종류 버튼이 달라집니다. <b>기존 RCS</b>는 <b>SMS · LMS · MMS · 템플릿 · LMS템플릿 · 이미지템플릿</b> 6가지, <b>통합 RCS</b>는 <b>SMS · LMS · MMS · 템플릿</b> 4가지입니다.<br><b>템플릿</b> 탭에는 <b>승인이 끝난 템플릿만</b> 나옵니다(확인 시점 <b>38건</b>). 한 줄을 고르면 오른쪽에 그 템플릿의 <b>실제 모양이 미리 보입니다</b>.<br><b class=\"hz-ex\">이 화면에서는</b> <code>모노_인증메시지</code> 를 골랐습니다. 메시지 베이스 ID는 <code>UBR.&lt;브랜드ID&gt;-…</code> 형태입니다." },
      body: [
        { t: "제목·내용 입력란이 없습니다", hi: ["템플릿 설정 정보"],
          d: "템플릿은 <b>문구가 이미 정해져 있어</b> 직접 쓰는 칸이 없습니다. 대신 위에 <b>템플릿 설정 정보</b> 영역이 생겨 <b>버튼을 확인</b>하고, <b>변수 입력</b>은 아래 <b>메시지 공통 정보</b> 안에 붙습니다." },
        { t: "버튼 설정 정보 확인 — 보기만 됩니다", hi: ["버튼 설정 정보 확인"],
          d: "템플릿에 설정돼 있는 버튼을 <b>확인만</b> 하는 칸입니다. 여기서 바꿀 수 없고, 바꾸려면 <b>템플릿 관리</b>에서 템플릿 자체를 수정해 다시 승인받아야 합니다.<br><b class=\"hz-ex\">이 템플릿은</b> <code>버튼 : 미사용</code> 입니다." },
        { t: "변수 이름은 템플릿이 정합니다", hi: ["변수 입력"],
          d: "직접 작성할 때의 <b>변수1~변수6</b>과 달리, 템플릿은 <b>만들 때 붙인 이름</b>이 그대로 나옵니다. 이 템플릿은 <code>cell1</code> 하나뿐이고, 템플릿에 따라 여러 개일 수도 있습니다. 칸에 값을 넣으면 발송 시 그 자리에 치환됩니다." },
        { t: "⚠ 글자수 한도는 90자 — 고정 문구까지 합쳐서 셉니다", hi: ["변수 입력"],
          d: "변수마다 <b>90자</b> 카운터가 붙고 맨 아래에 <b>총합</b> 카운터가 있습니다. 화면 경고 그대로 — <i>변수 가변내용 총 글자 합이 90자 이상일 경우 메시지가 전송되지 않습니다.</i><br><b class=\"hz-warn\">그런데 이 카운터는 변수 값만 셉니다.</b> 실제로 <b>90자</b>를 재는 대상은 <b>템플릿 고정 문구 + 변수 값</b>을 합친 본문 전체이고, 넘기면 <b>결과코드 55709(Description 최대 글자수 초과)</b>로 실패합니다.<br><b class=\"hz-warn\">그래서 고정 문구가 긴 템플릿은 변수를 거의 못 씁니다.</b> 실제로 고정 문구가 85자쯤 되는 템플릿에 변수를 <b>6자</b>(<code>고객</code>·<code>메시징</code>·<code>1</code>)만 넣었는데도 90자를 넘어 막혔습니다 — 승인이 끝난 템플릿이어도 그렇습니다(2026-09-08 실측).<br><b class=\"hz-ex\">이 화면의</b> <code>모노_인증메시지</code>는 고정 문구가 <b>25자 남짓</b>이라 여유가 넉넉합니다. <b>템플릿을 고를 때 고정 문구 길이부터 확인</b>하세요." },
        { t: "미리보기에서 치환 결과를 바로 확인합니다", hi: ["모노 문의 접수 안내"],
          d: "값을 넣으면 오른쪽 미리보기가 <b>치환된 결과</b>로 바뀝니다 — <code>{{name}}</code> 자리에 <code>홍길동</code>이 들어간 모습 그대로입니다. 템플릿이 가진 <b>제목(안내)</b>도 함께 보입니다. 미리보기 종류는 <b>openrichcard</b>로 표시됩니다." },
      ],
      actbtn: { t: "'(광고)' 표시와 수신거부번호가 다릅니다", hi: ["(광고)", "무료 수신거부번호"],
        d: "직접 작성과 달리 템플릿에서는 <b>'(광고)' 표시 여부 기본값이 미사용</b>이고, <b>무료 수신거부번호도 필수가 아닙니다</b>(칸 안내에 <i>(optional)</i>). 정보성 안내 템플릿이라 그렇습니다. <b>광고성으로 보낸다면</b> 표시를 <b>사용</b>으로 바꾸고 수신거부번호를 반드시 넣어야 합니다." },
    },
  },
  mms: {
    ...
    set: {
      ...
      write: [["'(광고)' 표시", "사용"], ["무료 수신거부번호", "08012345678"], ["만료 옵션", "40초"],
              ["카드", "3장"], ["이미지", "등록한 이미지 가져오기"], ["메시지 제목", "사용"],
              ["버튼 설정", "미사용"], ["Fallback", "사용"]],
      sendinfo: [["발송 형태", "즉시 발송"], ["발송량", "단건 · 20건 이하"], ["발송그룹ID", "GUIDE-20260831-02"]],
    },
    detail: {
      base: { d: "고른 규격에 따라 아래 종류 버튼이 달라집니다. <b>기존 RCS</b>는 <b>SMS · LMS · MMS · 템플릿 · LMS템플릿 · 이미지템플릿</b> 6가지, <b>통합 RCS</b>는 <b>SMS · LMS · MMS · 템플릿</b> 4가지입니다.<br>MMS 탭에는 <b>23가지</b>가 있습니다 — 세로형 2 · <b>슬라이드형 10</b>(Medium/Small × 2~6장) · SNS형 2 · 기본형 2 · 썸네일형 2 · 아이템 상세형 1 · 이미지&타이틀 강조형 2 · 이미지 강조형 2. <b>고른 종류에 따라 아래 입력 항목이 달라집니다.</b><br><b class=\"hz-ex\">이 화면에서는</b> <code>슬라이드형(Medium,3장)</code> 을 골랐고, 메시지 베이스 ID는 <code>CMwMhM0300</code> 입니다." },
      body: [
        { t: "카드가 3장 — 위쪽 1 · 2 · 3 으로 옮겨 다닙니다", hi: ["첫번째 카드"],
          d: "슬라이드형은 카드를 <b>옆으로 넘겨 보는</b> 형태입니다. 고른 종류가 <b>3장</b>이므로 카드 3개를 <b>각각</b> 채워야 합니다. 카드 제목 오른쪽의 <b>1 · 2 · 3</b> 버튼으로 옮겨 다니고, 지금 보이는 것은 <b>첫번째 카드</b>입니다." },
        { t: "이미지 — 두 가지 방법", hi: ["이미지"],
          d: "<b>등록한 이미지 가져오기</b> / <b>내 PC에서 가져오기</b> 중에서 고릅니다. 등록한 이미지를 쓰면 아래 <b>등록된 이미지로 업로드</b> 칸의 <b>[조회]</b>로 <b>파일 관리</b>에 올려 둔 이미지를 고릅니다.<br><b class=\"hz-ex\">이 화면에서는</b> 등록해 둔 <code>mono_intro_01.jpg</code> 를 골랐습니다." },
        { t: "유튜브 동영상 URL", hi: ["유튜브 동영상 URL"],
          d: "<b>사용</b>으로 바꾸면 이미지 대신 유튜브 주소를 넣을 수 있습니다. 이 예제는 <b>미사용</b>입니다." },
        { t: "카드 제목 · 내용", hi: ["메시지 제목", "메시지 내용"],
          d: "제목은 LMS와 같이 <b>사용 / 미사용</b>을 고르고 쓸 때 <b>30자</b>까지, 내용은 <b>1,300자</b>까지입니다. <b>내용만 필수(*)</b>입니다.<br>카드마다 따로 씁니다 — 1번 카드를 다 쓰면 위쪽 <b>2</b>를 눌러 다음 카드로 넘어갑니다." },
        { t: "버튼은 카드당 2개까지", hi: ["버튼 설정"],
          d: "SMS·LMS의 <b>액션 버튼 설정</b>과 달리 슬라이드형은 <b>미사용 / 1개 / 2개</b>입니다. 카드가 좁아 3개는 들어가지 않습니다.<br>이 예제는 <b>미사용</b>으로 뒀습니다." },
      ],
      actbtn: { t: "커스텀 변수는 카드마다", hi: ["커스텀 변수 설정"],
        d: "<b>변수1 ~ 변수6</b>을 카드 내용에 넣을 수 있습니다. 안내 그대로 — <i>변수의 길이를 포함하여 각 상품별 글자수를 초과하면 잘려서 전송됩니다.</i>" },
    },
  },
  lms: {
    ...
    detail: {
      base: { d: "고른 규격에 따라 아래 종류 버튼이 달라집니다. <b>기존 RCS</b>는 <b>SMS · LMS · MMS · 템플릿 · LMS템플릿 · 이미지템플릿</b> 6가지, <b>통합 RCS</b>는 <b>SMS · LMS · MMS · 템플릿</b> 4가지입니다. 버튼을 누르면 아래 목록이 바뀌고, <b>이 가이드에서도 실제 목록을 눌러 볼 수 있습니다.</b><br><b class=\"hz-ex\">이 화면에서는</b> <code>기존 RCS · LMS</code> 를 골랐고, 메시지 베이스 ID는 <code>SL000000</code> 입니다.<br>LMS 탭에는 <b>LMS · 기본형 · 기본형 타이틀 강조 · 문단형</b> 4가지가 있고, <b>고른 베이스에 따라 아래 입력 항목이 아예 달라집니다</b>(예: ‘기본형 타이틀 강조’는 메인 제목·로고 항목이 따로 생깁니다). 이 따라하기는 맨 위 <code>LMS</code> 기준입니다." },
      body: {
        t: "제목 · 내용 → 미리보기",
        hi: ["메시지 제목", "메시지 내용", "기존 RCS · LMS"],
        d: "LMS는 SMS와 달리 <b>제목</b>을 쓸 수 있고, 쓸지 말지를 <b>사용 / 미사용</b>으로 고릅니다. 기본값은 <b>사용</b>이고 <b>필수 항목은 아닙니다</b>.<br>· <b>사용</b> — 제목 칸이 열리고 <b>30자</b>까지. 미리보기 맨 윗줄에 굵게 나옵니다.<br>· <b>미사용</b> — <b>제목 칸 자체가 사라지고</b> 미리보기에서도 제목 줄이 빠집니다.<br>본문은 <b>1,300자</b>까지입니다(SMS는 제목 없이 본문 100자).<br><b class=\"hz-ex\">이 화면에서는</b> 제목 <code>서비스 점검 안내</code>, 본문에 점검 안내문을 넣었습니다.",
      },
    },
  },
};
...
function hzSteps(type) {
  ...
  const tov = HZ_TYPES[t].tip || {};      // 유형마다 주의사항이 다를 수 있다
  HZ_STEP_CACHE[t] = HZ_GROUPS.map((g) => {
    ...
    return {
      ...
      tip: tov[g.id] || g.tip || (srcs[0] ? srcs[0].tip : ""),
      /* override 가 객체면 그 항목만 바꾸고, 배열이면 그 자리에 여러 항목을 편다
         (MMS 처럼 한 칸이 여러 설명으로 쪼개지는 경우) */
      detail: srcs
        .reduce((a, x) => a.concat(x.detail || []), [])
        .reduce((a, d) => {
          const o = d.id ? dov[d.id] : null;
          if (!o) return a.concat([d]);
          return a.concat(Array.isArray(o) ? o : [Object.assign({}, d, o)]);
        }, []),
    };
  });
