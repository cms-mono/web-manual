  ...
  A["communis"] = {
    ...
    features: [
      ...
      { id: "setup", group: "시작하기", name: "② 서비스 신청 · 설정",
        intro: "회원가입 후 로그인하여 대시보드에서 진행하는 단계입니다. 청구계정 등록 → 청구정보 선택 → 서비스 신청·API 선택 → 서비스 등록정보 조회 → 발송 전 사전 준비 순으로 진행합니다.",
        steps: [
          {
          ...
          {
            title: "청구정보 선택 (선불 · 후불 · 월정액)",
            body: "<b>대시보드 &gt; 서비스 관리 &gt; 서비스 신청하기</b> 화면에서 <b>먼저 청구정보(과금 방식)</b>를 선택합니다. <b>선택한 청구 방식에 따라 이후 표시되는 API 목록이 달라집니다.</b>",
            table: {
              cols: ["청구 방식", "설명", "선택 시 신청 가능한 API"],
              colWidths: ["16%", "50%", "34%"],
              rows: [
                ["선불", "캐쉬를 미리 <b>충전</b>해야 API를 정상 이용할 수 있음", "메시징 API"],
                ["후불", "사용한 만큼 <b>후청구</b>", "메시징 API (문자·국제SMS·2FA·카카오·이메일·RCS·APP PUSH)"],
                ["월정액", "월 <b>정액 요금</b> 상품", "<b>080 수신거부</b> (080 API)"],
              ],
            },
            list: [
              "<b>고객 ID</b> — 청구고객 ID가 표시됨",
              "<b>청구계정</b> — 드롭다운에서 사용할 청구계정 선택",
              "청구고객·청구계정이 <b>없을 경우에만</b> [고객 등록] / [계정 등록] 버튼이 표시되며, 클릭하면 등록 페이지로 이동",
              "<b>월정액</b> 선택 시 하단에 <b>월정액 API 선택</b> 영역이 나타나고, 원하는 API(080 수신거부)를 켜면 추가됨",
            ],
            shot: { img: "assets/communis/svc-4-billing-postpaid.png", url: "communis.kt.co.kr/service/apply", label: "청구정보 — 후불 선택 (고객 ID · 청구계정)" },
            note: "080 수신거부 번호 청약·관리 API는 <b>부록 &gt; 080 수신거부</b> 탭을 참고하세요. 청구계정 등록은 <b>앞 단계(청구계정 등록)</b>에서 먼저 완료해야 합니다.",
          },
          {
            title: "서비스 신청 · API 선택",
            body: "청구정보를 선택했으면 같은 화면에서 사용할 API를 신청합니다.",
            list: [
            ...
            note: "⚠ 위 <b>메시징 API 목록은 앞 단계에서 청구정보를 ‘후불’(또는 선불)로 선택했을 때</b> 표시됩니다. <b>월정액</b>을 선택하면 <b>080 수신거부</b>가 대신 표시됩니다(다음 단계 참고).",
          },
          {
            title: "월정액 선택 시 — 080 수신거부",
            body: "청구정보에서 <b>월정액</b>을 선택하면 하단에 <b>월정액 API 선택</b> 영역이 나타납니다. 여기서 <b>080 수신거부</b>를 켜면 서비스에 추가됩니다.",
            shot: { img: "assets/communis/svc-5-billing-monthly-080.png", url: "communis.kt.co.kr/service/apply", label: "청구정보 — 월정액 선택 시 080 수신거부 API 표시" },
            note: "월정액에서는 <b>메시징 API 목록이 표시되지 않습니다</b>. 문자·카카오·RCS 등 메시징 채널은 <b>선불 또는 후불</b>로 신청하세요.",
          },
          {
            title: "서비스 등록 정보 조회",
      ...
      { id: "webKakao", group: "웹 발송", name: "웹 카카오 발송",
        ...
        steps: [
          ...
          },
          {
            ...
            shot: { img: "assets/communis/web-kakao-tpl.png", url: "communis.kt.co.kr/ums/user/send/alimtalk/templateList.do", label: "알림톡 템플릿 목록 — 승인된 템플릿만 카드로 표시 (좌측 상단 협력사 전용 영역은 지웠습니다)" },
      ...
      { id: "webResult", group: "웹 발송", name: "주소록 · 발송 결과·통계",
        ...
        steps: [
          ...
          },
          {
            ...
            shot: { img: "assets/communis/web-result-stat.png", url: "communis.kt.co.kr/ums/admin/statistics/send/list.do", label: "통계 > 발송통계 — 통합/웹발송/API 탭 · 월별 채널별 성공·실패 (좌측 상단 협력사 전용 영역은 지웠습니다)" },
      ...
      { id: "global", group: "API 연동 · 부록", name: "국제 SMS",
        intro: "해외 수신자에게 SMS를 발송하는 국제 SMS API입니다. 발송 전 서비스 신청·Sender ID 등록 등 준비가 필요하므로, 아래 ‘도입 절차’ 단계를 먼저 확인하세요. 기준 API: CPaaS_globalSms(v1.0). (규격서 v3.0.29 §4)",
        steps: [
          {
            title: "개요 & 엔드포인트",
          ...
          {
            title: "도입 절차 — 전체 진행 순서",
            body: "국제 SMS는 <b>커뮤니즈 가입만으로는 발송할 수 없습니다.</b> <b>국제 SMS 서비스를 별도로 신청</b>해야 하고, 국가에 따라 <b>Sender ID 사전 등록</b>이 필요합니다. 전체 흐름은 다음과 같습니다.",
            list: [
              "① 커뮤니즈 가입 → 청구계정 등록 → <b>「국제 SMS」 서비스 신청</b> → <b>API Key 발급</b>",
              "② <b>연동 테스트(국내 → 국내)</b> — 해외 단말·Sender ID 없이 즉시 가능",
              "③ 발송 대상 국가 · 월 예상 발송량 · 발송 목적을 <b>KT 양식(엑셀)으로 회신</b>",
              "④ KT: 국가별 규정 검토 + <b>Sender ID 등록 의무국 여부</b> 판단",
              "⑤ Sender ID 필요 시 → <b>KT 대행 등록</b> (국가별 요청 서류 제출)",
              "⑥ 필요 국가는 <b>템플릿 등록</b>",
              "⑦ 연동(API 또는 Agent) 완료 후 <b>실발송</b>",
            ],
            note: "출처: KT 메시징플랫폼서비스팀 안내(2026-09-08) + 커뮤니즈 가입·서비스 신청 가이드.",
          },
          {
            title: "도입 절차 ① — 가입 + 국제 SMS 서비스 등록 (선행 필수)",
            body: "커뮤니즈는 <b>서비스별로 개별 신청</b>하는 구조입니다. 이미 SMS/LMS를 사용 중인 고객사도 <b>국제 SMS는 다시 신청</b>해야 합니다.",
            table: {
              cols: ["순서", "항목", "내용"],
              colWidths: ["8%", "26%", "66%"],
              rows: [
                ["①", "회원가입", "회원 유형 <b>법인사업자</b> / 서류 4종(사업자등록증 사본·법인 인감증명서·위임장·담당자 신분증 사본)"],
                ["②", "청구계정 등록", "<b>결제서비스 &gt; 청구계정 정보</b>에서 등록. <b>서비스 신청보다 반드시 먼저</b> 완료"],
                ["③", "국제 SMS 서비스 신청", "<b>대시보드 &gt; 서비스 관리 &gt; 서비스 신청하기</b>에서 <b>「국제 SMS」</b> 선택 (이용약관 동의)"],
                ["④", "발신정보 등록", "국제 SMS 자체는 <b>발신번호 등록이 불필요</b>합니다. 다만 발송 결과를 받으려면 <b>발신정보 &gt; 국제SMS 웹훅</b> URL을 등록해야 합니다"],
                ["⑤", "연동 방식 결정", "<b>API 직접 연동</b> 또는 <b>Agent 연동</b>. 국제 SMS는 커뮤니즈 서비스라 <b>커뮤니즈를 연동하는 모노 에이전트(M2X ONE · Odyssey)로 발송할 수 있습니다</b>. 설정 방법은 각 에이전트 매뉴얼을 참고하세요. 어느 쪽이든 <b>KT ↔ 모노 협의</b>가 선행되어야 합니다"],
              ],
            },
            note: "<b>기존 고객사 점검 3종</b> — 청구계정 등록 有 / <b>「국제 SMS」 서비스 신청</b> 有 / <b>국제SMS 웹훅</b> 등록 有. <b>하나라도 빠지면 발송이 실패합니다.</b> 웹훅 등록 위치는 <b>시작하기 &gt; 발신정보</b> 탭을 참고하세요.",
          },
          {
            title: "도입 절차 ② — 연동 테스트 (국내 → 국내)",
            body: "국제 SMS 채널로 <b>한국 발신번호 → 한국 수신번호</b>로 발송해 보는 방식입니다. <b>해외 단말도, Sender ID 사전 등록도 필요 없어</b> 준비물 없이 즉시 가능합니다. 해외 발송 준비(단말 확보·국가 확정·Sender ID 심사)는 <b>빨라야 수 주</b>가 걸리므로, 그동안 <b>연동 개발을 먼저 끝내 두고</b> 준비가 끝나면 <b>수신번호만 바꿔 실발송</b>하기 위한 단계입니다. 국제 SMS 단가표에 대한민국(KR)이 정식 등재되어 있어 규정상 유효한 발송입니다.",
            tables: [
              {
                label: "이 단계에서 검증되는 것",
                cols: ["항목", "내용"],
                colWidths: ["30%", "70%"],
                rows: [
                  ["계정 · 인증", "가입 상태, API ID/PW(Basic 인증) 정상 여부"],
                  ["서비스 활성화", "계정에 <b>국제 SMS</b>가 열려 있는지"],
                  ["요청 규격", "국가코드 포함 수신번호 형식(<code>8210…</code>), 국내 문자와 다른 파라미터 세트"],
                  ["Agent 설정", "Agent 연동 시 국제 SMS 발송 설정"],
                  ["결과 수신", "응답 구조(messages[]) 및 웹훅 결과 수신 경로"],
                  ["과금 집계", "국제 SMS 과금 라인 정상 집계"],
                ],
              },
              {
                label: "이 단계로 검증되지 않는 것 (2차 테스트 필요)",
                cols: ["항목", "확인 가능 여부"],
                colWidths: ["70%", "30%"],
                rows: [
                  ["현지 통신사 라우팅 · 도달률", "✗"],
                  ["Sender ID가 실제로 표시되는 형태", "✗"],
                  ["해당 국가 규정 · 템플릿 심사 통과 여부", "✗"],
                  ["현지 단말에서의 한글/특수문자 인코딩 표시", "✗"],
                  ["국가별 지연시간", "✗"],
                ],
              },
            ],
            note: "⚠ 고객사 안내 시 <b>‘1차는 연동 검증, 실제 도달 검증은 발송 국가 확정 후 2차로 별도 진행’</b> 이라고 명확히 구분해 주세요.",
          },
          {
            title: "도입 절차 ②-1 — 테스트 원리 (고객 설명용)",
            body: "고객사에서 <b>“한국 번호로 보내는데 왜 국제 SMS 테스트가 되느냐”</b>는 질문이 가장 많이 나옵니다. 다음과 같이 설명하면 됩니다.",
            list: [
              "고객사 서버에서 한국 휴대폰 번호로 발송하더라도, 메시지가 전달되는 <b>발송·수신 경로는 국내 SMS망이 아니라 국제 SMS망</b>입니다.",
              "국내에서 발신된 메시지가 <b>국제 SMS 플랫폼을 거쳐 처리된 뒤</b> 최종적으로 한국 휴대폰 번호로 수신되는 구조입니다.",
              "따라서 일반 국내 SMS와는 <b>다른 경로</b>를 이용하며, 수신 단말에서는 <b>해외에서 수신된 국제 SMS 형태로 인식</b>됩니다.",
            ],
            note: "⚠ <b>흔한 오해</b> — “해외로 나갔다가 착신전환으로 다시 국내로 돌아오는 방식”이 아닙니다. 국내에서 발신하되 <b>국제 SMS 경로(플랫폼)를 그대로 태우는</b> 것입니다. (KT 메시징플랫폼서비스팀 회신, 2026-09-10)",
          },
          {
            title: "도입 절차 ②-2 — 테스트 환경 구성 · 발신번호 표시",
            body: "Agent 연동으로 테스트할 경우의 구성 순서입니다.",
            list: [
              "① 커뮤니즈 가입 후 <b>국제 SMS 서비스 신청</b>",
              "② 계정 생성 후 <b>국제 SMS API Key 발급</b>",
              "③ 발급받은 <b>API Key를 에이전트에 설정</b>",
              "④ 고객사 서버에 <b>에이전트 설치 및 연동</b> (모노 지원)",
              "⑤ <b>한국 휴대폰 번호 대상으로 국제 SMS 발송 테스트</b> 진행",
            ],
            tables: [
              {
                label: "테스트 시 발신번호(Sender ID) 표시 정책",
                cols: ["항목", "내용"],
                colWidths: ["30%", "70%"],
                rows: [
                  ["한국의 Sender ID 정책", "해외 국가들과 달리 <b>알파벳 Sender ID를 허용하지 않고</b>, <b>숫자 형태의 발신번호만</b> 허용"],
                  ["숫자 발신번호 사용", "숫자 형태의 발신번호도 <b>별도 등록 절차</b>를 거쳐야 사용 가능"],
                  ["테스트 발송의 경우", "테스트 목적이므로 <b>발신번호 등록 절차를 진행하지 않음</b> → 수신 단말에 <b>랜덤 숫자 형태의 발신번호</b>가 표시됨"],
                ],
              },
            ],
            note: "⚠ 고객사가 “발신번호가 모르는 숫자로 온다”고 문의하면 <b>테스트 발송의 정상 동작</b>입니다. 실서비스에서 특정 발신번호를 쓰려면 <b>숫자 발신번호 등록</b>이 필요합니다. (KT 회신, 2026-09-08 · 2026-09-10)",
          },
          {
            title: "도입 절차 ③ — 해외 발송 준비물 · KT 회신 항목",
            tables: [
              {
                label: "실제 해외 발송 준비물",
                cols: ["준비 항목", "담당"],
                colWidths: ["70%", "30%"],
                rows: [
                  ["해외 수신 가능한 휴대폰 또는 테스트 환경 확보", "고객사"],
                  ["발송 대상 국가 확정", "고객사"],
                  ["Sender ID 사전 등록 (등록 의무 국가인 경우)", "<b>KT 대행</b> (고객사는 서류 제출)"],
                ],
              },
              {
                label: "KT 회신 항목 — 「회신요청_국제 SMS 국가별_월별 발송량.xlsx」",
                cols: ["항목", "작성 요령"],
                colWidths: ["26%", "74%"],
                rows: [
                  ["발송 대상 국가", "국가별로 기재"],
                  ["월 예상 발송량", "국가별 · 월별"],
                  ["발송 목적", "① 사내 포털 로그인 등 인증번호(OTP) 발송 ② 시스템 알림 문자 발송 ③ 해외 고객 대상 안내 메시지 발송 등"],
                ],
              },
            ],
            note: "Sender ID 등록 의무 국가 여부와 국가별 필요 서류는 <b>발송 국가가 확정된 후</b> KT가 판단해 별도 안내합니다. <b>발송 목적은 향후 국가별 규정 검토 · 템플릿 등록의 판단 근거</b>이므로 가능한 한 상세히 기재해야 하며, 뭉뚱그려 작성하면 절차가 지연됩니다.",
          },
          {
            title: "도입 절차 ④ — 국제 SMS 청약 프로세스 (KT 공식)",
            body: "테스트를 마치고 실제 청약으로 넘어갈 때 KT가 안내하는 공식 절차입니다. <b>[ ]</b> 안은 담당 주체입니다.",
            table: {
              cols: ["순서", "절차", "담당"],
              colWidths: ["8%", "68%", "24%"],
              rows: [
                ["①", "발송 국가 및 <b>국가별·월별 발송량</b> 확인", "고객사 · KT"],
                ["②", "Communis 홈페이지 가입 및 서비스 신청", "<b>고객사</b>"],
                ["③", "<b>Sender ID 등록 정보</b> 준비 및 KT 전달 (필요 양식 별도 전달)", "<b>고객사</b>"],
                ["④", "국가별·통신사별 <b>Sender ID 등록 대행</b> 및 <b>국가별 특이 규제사항</b> 고객사 안내", "<b>KT</b>"],
                ["⑤", "국가별·통신사별 <b>승인 절차</b> 진행", "<b>KT</b>"],
                ["⑥", "고객사 <b>API 연동</b>", "고객사 (KT 지원)"],
              ],
            },
            note: "국가마다 규제가 달라 ④~⑤에서 시간이 걸립니다. 발송 국가가 확정되기 전에는 소요 기간을 확답하지 마세요.",
          },
          {
            title: "기존 스마트메시지BIZ 고객사가 국제 SMS만 추가하는 경우",
            body: "국내 발송은 <b>스마트메시지BIZ를 그대로 유지</b>하면서 <b>국제 문자만 Communis 국제 SMS로</b> 이용할 수 있습니다. 실제로 가장 많은 문의 유형입니다.",
            table: {
              cols: ["항목", "가능 여부 · 유의사항"],
              colWidths: ["32%", "68%"],
              rows: [
                ["국내는 스마트메시지BIZ 유지", "<b>가능</b>. 국내 발송 구성은 그대로 두고 국제 SMS만 별도 이용"],
                ["기존 협력사 유지", "<b>가능</b>. 기존 협력사를 그대로 두고 동일 협력사를 통해 Communis 국제 SMS 이용 가능"],
                ["API 연동", "⚠ <b>별도 연동 필요</b>. 스마트메시지BIZ API와 Communis 국제 SMS API는 <b>별개의 서비스</b>이므로, 국제 SMS 발송을 위한 <b>Communis API 연동 작업이 추가</b>로 발생"],
              ],
            },
            note: "즉 <b>계약·협력사는 이어갈 수 있지만 연동은 새로 해야 한다</b>는 점을 초기에 못 박아 두어야 개발 일정 협의가 꼬이지 않습니다. (KT 회신, 2026-07-24)",
          },
          {
            title: "고객 문의 대응 — 자주 나오는 질문",
            body: "국제 SMS 문의에서 반복되는 질문과 답변입니다.",
            table: {
              cols: ["질문", "답변"],
              colWidths: ["38%", "62%"],
              rows: [
                ["커뮤니즈를 이미 쓰고 있는데 바로 되나요?", "<b>아니오.</b> [서비스 신청하기]에서 <b>「국제 SMS」를 추가로 신청</b>해야 합니다."],
                ["테스트부터 해보고 싶습니다.", "<b>국내 → 국내</b> 발송으로 가능합니다. 해외 단말·Sender ID가 필요 없습니다."],
                ["Sender ID 등록은 저희가 하나요?", "<b>KT가 대행</b>합니다. 고객사는 국가별 요청 서류만 제출하면 됩니다."],
                ["어느 국가가 Sender ID 등록이 필요한가요?", "<b>발송 국가가 확정된 후</b> KT가 판단해 안내합니다."],
                ["한국 번호로 보내는데 국제 SMS 테스트가 되나요?", "됩니다. 발송·수신 <b>경로가 국내 SMS망이 아닌 국제 SMS망</b>이라, 수신 단말에서는 해외에서 온 국제 SMS로 인식됩니다."],
                ["해외로 나갔다가 착신전환으로 돌아오는 방식인가요?", "<b>아닙니다.</b> 국내에서 발신하되 국제 SMS 플랫폼을 그대로 경유하는 구조입니다."],
                ["테스트인데도 가입·서비스 신청이 필요한가요?", "<b>필요합니다.</b> 테스트 발송도 커뮤니즈 가입 후 국제 SMS 서비스 신청을 마쳐야 가능합니다."],
                ["테스트 문자의 발신번호가 모르는 숫자로 표시됩니다.", "<b>정상입니다.</b> 테스트는 발신번호 등록 절차를 생략하므로 랜덤 숫자가 표시됩니다. 한국은 알파벳 Sender ID를 허용하지 않으며, 숫자 발신번호도 별도 등록이 필요합니다."],
                ["국내는 스마트메시지BIZ 그대로 쓰고 국제만 추가할 수 있나요?", "<b>가능합니다.</b> 기존 협력사도 유지할 수 있습니다. 단 두 서비스의 API가 별개라 <b>Communis API 연동 작업이 추가로 필요</b>합니다."],
                ["모노 에이전트로 연동 가능한가요?", "<b>가능합니다.</b> <b>M2X ONE · Odyssey 모두 국제 SMS를 지원</b>합니다. (McsAgent는 스마트메시지BIZ 전용이라 불가) 설정은 각 에이전트 매뉴얼을 참고하시고, <b>KT ↔ 모노 협의</b>가 선행됩니다."],
                ["단가는 어떻게 되나요?", "국가별로 상이합니다. 커뮤니즈 <b>고객센터 &gt; 공지사항</b>의 국제 SMS 단가표를 참고하세요(251개국, 매월 현행화)."],
              ],
            },
          },
          {
