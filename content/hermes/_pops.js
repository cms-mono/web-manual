...
/* rcs.hermes.kt.com 선택 팝업들 — 실제 마크업·스타일
   ...
   · '레이아웃'은 목록이 비어 있고 KT RCS에 지원 여부 문의 중이라 유형에서 제외했다.
...
.hz-real .vue-js-switch .v-switch-core .v-switch-button { display: block; position: absolute; overflow: hidden; top: 0px; left: 0px; border-radius: 100%; background-color: rgb(255, 255, 255); z-index: 2; }`;
  ...
  POPS.base = `<div class="popup fild-list fild-base preview"><div class="popup-header" style="margin-bottom: 10px;"><h3>메시지 종류를 선택해주세요</h3></div><div class="popup-body"><fieldset><div class="tab-menu mg-t25"><ul><li class="current"><a>기존 RCS</a><div class="tooltip"><p>삼성 단말 이용자에게만 RCS 전송이 가능하지만, 가독성이 높고 효과적인 메시지</p></div></li><li class=""><a>통합 RCS</a><div class="tooltip"><p>삼성, 아이폰 등 단말의 구분 없이 모든 고객에 RCS 메시지를 전송할 수 있어 편리하고 효율적인 메시지</p></div></li></ul></div><div><div class="menu-btn mg-t10"><div class="menu-area" style="justify-content: center;"><div class="menu-item selected">SMS</div><div class="menu-item">LMS</div><div class="menu-item">MMS</div><div class="menu-item">템플릿</div><div class="menu-item">LMS템플릿</div><div class="menu-item">이미지템플릿</div></div></div><div class="flex mg-t10" style="width: 630px; margin: 10px auto 0;"><input type="text" placeholder="검색어를 입력해주세요." value="" readonly=""><button type="button" class="btn btn-sm4 btn-gray fx-init">검색</button></div></div></fieldset><div class="flex mg-t10"><div class="table-wrap scroll-y" style="flex:1"><table class="table-data"><caption>메시지 종류 선택</caption><colgroup><col style="width: 42px;"><col style="width: auto;"></colgroup><thead><tr><th>번호</th><th>메시지 종류</th></tr></thead><tbody><tr style="background-color:#e8f7fa;"><td>1</td><td>SMS</td></tr></tbody></table></div><div class="message-preview fx-init" style="width: 320px;"><div class="card table-type preview-wrap"><div class="card-header"><h2>미리보기</h2></div><div class="card-body"><div class="preview-area template desc custom-scroll"><div class="card-type"><div class="inner"><div class="cont"><div class="body-area"><div class="txt-area"><p class="txt align-c">미리보기 화면</p></div></div></div></div></div></div></div></div></div></div></div><div class="popup-footer"><div class="btn-area"><button type="button" class="btn btn-sm2">취소</button><button type="button" class="btn btn-sm2 btn-primary">확인</button></div></div></div>`;
  ...
  var BASE_SETS = {
    "기존 RCS": {
      types: ["SMS", "LMS", "MMS", "템플릿", "LMS템플릿", "이미지템플릿"],
      lists: {
        ...
        "LMS템플릿": `<tr><td colspan="2" style="text-align:left;padding:8px 10px;color:#e2711d;">📢 LMS 템플릿 명세서B형/C형은 현재 KT RCS 포탈에서 제공되지 않고 있으니 참고 부탁드립니다.</td></tr><tr style="background-color:#e8f7fa;"><td>1</td><td>LMS 테스트 템플릿</td></tr>`,
